...
class computational_sequence():
    ...
    def _remove_id(self, entry_id, purge=False):
        if purge == False:
            ...
            keys_to_del = [key for key in list(
                self.keys()) if key[:len(entry_id)] == entry_id]
            for key_to_del in keys_to_del:
                del self.data[key_to_del]
        else:
            log.error(
                "Purge received wrong argument type. Exiting ...!", error=True)
    ...
    def __initialize_from_csd(self, resource, destination, validate):
        if validators.url(resource):
            ...
            # user has chosen a different directory
                ...
                destination = os.path.join(
                    destination, resource.split(os.sep)[-1])
            # .csd cannot be in the destination as we don't allow name changes when downloading
            else:
                log.error(
                    "Destination needs to be a folder where the downloaded computational sequence is stored. Exiting ...!", error=True)
        ...
        if type(metadata) is not dict:
            log.error("Metadata not in correct format for %s. Exiting ...!" %
        ...
        if validate:
            ...
            log.warning(
                "Validation of the computational sequence skipped by user request.")
    ...
    def __initialize(self, resource, destination, validate):
        ...
        if hasattr(self, 'h5handle'):
            log.error("<%s> computational sequence already initialized ..." %
                      self.metadata["root name"], error=True)
    ...
    def __check_format(self, error=True):
        if not hasattr(self, 'metadata') or not hasattr(self, 'data'):
            log.error(
                "computational sequence is blank (data or metadata is missing)")
        log.status("Checking the integrity of the <%s> computational sequence ..." %
                   self.metadata["root name"])
        # TODO: hash check not implemented yet
        datavalid = validate_data_format(
            self.data, self.metadata["root name"], verbose=False)
        metadatavalid = validate_metadata_format(
            self.metadata, self.metadata["root name"], verbose=False)
        if datavalid and metadatavalid:
            log.success("<%s> computational sequence is valid!" %
                        self.metadata["root name"])
    ...
    def complete_all_missing_metadata(self):
        ...
        missings = [x for (x, y) in zip(featuresetMetadataTemplate, [
            metadata in self.metadata.keys() for metadata in featuresetMetadataTemplate]) if y is False]
        # python2 vs python 3
        ...
        for missing in missings:
            self.metadata[missing] = log.status("Please input %s%s: " % (
                missing, root_name_ext), require_input=True)
    ...
    def deploy(self, destination, compression="gzip", compression_opts=9, full_chunk_shape=True):
        ...
        self.__check_format()
        log.status("Deploying the <%s> computational sequence to %s" %
                   (destination, self.metadata['root name']))
        ...
        write_CSD(self.data, self.metadata, self.metadata["root name"], destination,
                  compression=compression, compression_opts=compression_opts, full_chunk_shape=full_chunk_shape)
        self.main_file = destination
    ...
    def bib_citations(self, outfile=None):
        ...
        if self.metadata is None or self.metadata == {}:
            log.error(
                "Metadata is not set for <%s> computational sequence" % self.root_name)
        outfile.write('Computational Sequence <%s> bib: ' %
                      self.root_name+self.metadata['featureset bib citation']+'\n\n')
        outfile.write('Dataset <%s> bib: ' %
                      self.metadata["dataset name"]+self.metadata['dataset bib citation']+'\n\n')
