...
def read_CSD(resource, destination=None):
    ...
    if (resource is None):
        raise log.error(
            "No resource specified for computational sequence!", error=True)
    if os.path.isfile(resource) is False:
        log.error("%s file not found, please check the path ..." %
                  resource, error=True)
    try:
        h5handle = h5py.File('%s' % resource, 'r')
    except:
        raise log.error(
            "%s resource is not a valid hdf5 computational sequence format ..." % resource, error=True)
...
def write_CSD(data, metadata, rootName, destination, compression, compression_opts, full_chunk_shape):
    ...
    if compression is not None:
        log.advise("Compression with %s and opts -%d" %
    ...
    for vid in data:
        ...
        if compression is not None:
            vidHandle.create_dataset(
                "features", data=data[vid]["features"], compression=compression, compression_opts=compression_opts)
            vidHandle.create_dataset(
                "intervals", data=data[vid]["intervals"], compression=compression, compression_opts=compression_opts)
        else:
    ...
    for metadataKey in metadata.keys():
        ...
        metadataHandle[metadataKey][0] = cast_operator(
            json.dumps(metadata[metadataKey]))
    writeh5Handle.close()

    log.success("<%s> computational sequence metadata successfully wrote to %s" % (
        rootName, destination))
        ...
        for key in metadata_.keys():
            try:
                ...
                try:
                    ...
                    log.error(
                        "Metadata %s is in wrong format. Exiting ...!" % key)
        return metadata
