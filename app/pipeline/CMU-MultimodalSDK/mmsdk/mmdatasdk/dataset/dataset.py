...
class mmdataset:
    ...
    def __init__(self, recipe, destination=None):
        ...
        if type(recipe) is str:
            ...
            computational_sequence_list = [f for f in listdir(
                recipe) if isfile(join(recipe, f)) and f[-4:] == '.csd']
            for computational_sequence_fname in computational_sequence_list:
                ...
                self.computational_sequences[this_sequence.metadata["root name"]
                                             ] = this_sequence
            ...
            log.error(
                "Cannot create a dataset with the given recipe. Exiting ...!", error=True)
    ...
    def add_computational_sequences(self, recipe, destination):
        for entry, address in recipe.items():
            if entry in self.computational_sequences:
                log.error(
                    "Dataset already contains <%s> computational sequence ..." % entry)
            self.computational_sequences[entry] = computational_sequence(
                address, destination)
    ...
    def unify(self, active=True):
        ...
        for seq_key in list(self.computational_sequences.keys()):
            all_keys[seq_key] = [vidid.split(
                "[")[0] for vidid in self.computational_sequences[seq_key].data.keys()]

        valids = set.intersection(*[set(all_keys[x]) for x in all_keys])
        violators = set()
        for seq_key in list(self.computational_sequences.keys()):
            violators = violators.union(set([vidid.split(
                "[")[0] for vidid in self.computational_sequences[seq_key].data.keys()])-valids)

        if len(violators) > 0:
            for violator in violators:
                log.error(
                    "%s entry is not shared among all sequences, removing it ..." % violator, error=False)
                if active == True:
                    self.remove_id(violator, purge=True)
        if active == False and len(violators) > 0:
            log.error("%d violators remain, alignment will fail if called ..." % len(
                violators), error=True)
    ...
    def hard_unify(self, active=True):
        ...
        for seq_key in list(self.computational_sequences.keys()):
            all_keys[seq_key] = [
                vidid for vidid in self.computational_sequences[seq_key].data.keys()]

        valids = set.intersection(*[set(all_keys[x]) for x in all_keys])
        for seq_key in list(self.computational_sequences.keys()):
            hard_unify_compatible = all(
                ["[" in vidid for vidid in self.computational_sequences[seq_key].data.keys()])
            if hard_unify_compatible is False:
                log.error(
                    "Hard unify can only be done on aligned computational sequences, %s violated this ... Exiting ..." % seq_key)
            violators = set(
                [vidid for vidid in self.computational_sequences[seq_key].data.keys()])-valids
            for violator in violators:
                if active == True:
                    log.error(
                        "%s entry is not shared among all sequences, removing it ..." % violator, error=False)
    ...
    def align(self, reference, collapse_functions=None, replace=True):
        ...
        log.status(
            "Pre-alignment based on <%s> computational sequence started ..." % reference)
        relevant_entries = self.__get_relevant_entries(reference)
        log.status("Alignment starting ...")

        pbar = log.progress_bar(
            total=len(refseq.keys()), unit=" Computational Sequence Entries", leave=False)
        pbar.set_description("Overall Progress")
        for entry_key in list(refseq.keys()):
            pbar_small = log.progress_bar(
                total=refseq[entry_key]['intervals'].shape[0], unit=" Segments", leave=False)
                ...
                for otherseq_key in list(self.computational_sequences.keys()):
                    if otherseq_key != reference:
                        intersects, intersects_features = self.__intersect_and_copy(
                            ref_time, relevant_entries[otherseq_key][entry_key], epsilon)
                    else:
                        intersects, intersects_features = refseq[entry_key]['intervals'][i,
                                                                                         :][None, :], refseq[entry_key]['features'][i, :][None, :]
                    ...
                    if type(collapse_functions) is list:
                        intersects, intersects_features = self.__collapse(
                            intersects, intersects_features, collapse_functions)
                    if (intersects.shape[0] != intersects_features.shape[0]):
                        log.error("Dimension mismatch between intervals and features when aligning <%s> computational sequences to <%s> computational sequence" % (
                            otherseq_key, reference), error=True)
                    aligned_output[otherseq_key][entry_key+"[%d]" % i] = {}
                    aligned_output[otherseq_key][entry_key +
                                                 "[%d]" % i]["intervals"] = intersects
                    aligned_output[otherseq_key][entry_key+"[%d]" %
                                                 i]["features"] = intersects_features
                pbar_small.update(1)
            pbar_small.close()
            pbar.update(1)
        pbar.close()
        ...
        if replace is True:
            ...
            newdataset.__set_computational_sequences(
                aligned_output, metadata_copy=False)
            return newdataset
    ...
    def __collapse(self, intervals, features, functions):
        ...
        try:
            new_features = numpy.concatenate(
                [function(intervals, features) for function in functions], axis=0)
            if len(new_features.shape) == 1:
                new_features = new_features[None, :]
        except:
            log.error(
                "Cannot collapse given the set of functions. Please check for exceptions in your functions ...", error=True)
    ...
    def revert(self, replace=True):
        ...
        if len(self.keys()) == 0:
            log.error(
                "The dataset contains no computational sequences ... Exiting!", error=True)
        ...
        if len(all_keys) == 0:
            log.error(
                "No entries in computational sequences or removed during unify ... Exiting!")
        ...
        for key in all_keys:
            ...
            unique_unnumbered_entries[key.split(
                '[')[0]].append(int(key.split('[')[1][:-1]))

        pbar = tqdm(total=len(unique_unnumbered_entries.keys()),
                    unit=" Unique Sequence Entries", leave=False)
        ...
        for key in unique_unnumbered_entries.keys():
            ...
            for cs_key in reverted_dataset.keys():
                intervals = numpy.concatenate([self[cs_key][str('%s[%d]' % (
                    key, i))]["intervals"] for i in unique_unnumbered_entries[key]], axis=0)
                features = numpy.concatenate([self[cs_key][str('%s[%d]' % (
                    key, i))]["features"] for i in unique_unnumbered_entries[key]], axis=0)
                reverted_dataset[cs_key][key] = {
                    "intervals": intervals, "features": features}
            pbar.update(1)
        pbar.close()
        ...
        if replace is True:
            ...
            newdataset = mmdataset({})
            newdataset.__set_computational_sequences(
                reverted_dataset, metadata_copy=False)
            return newdataset

    def impute(self, ref_key, imputation_fn=numpy.zeros):
        log.status("Imputation called with function: %s ..." %
                   str(imputation_fn.__name__))
        other_keys = list(self.keys())
        other_keys.remove(ref_key)
        other_keys_dims = {x: list(
            self[x][list(self[x].keys())[0]]["features"].shape[1:]) for x in other_keys}
        pbar = tqdm(total=len(self[ref_key].keys(
        )), unit=" Reference Computational Sequence Entries", leave=False)
        ...
        for seg_key in self[ref_key].keys():
            for other_key in other_keys:
                try:
                    ...
                    self[other_key][seg_key] = {"intervals": self[ref_key][seg_key]["intervals"], "features": imputation_fn(
                        [num_entries]+other_keys_dims[other_key])}
    ...
    def __set_computational_sequences(self, new_computational_sequences_data, metadata_copy=True):
        ...
        old_metadata = {m: self.computational_sequences[m].metadata for m in list(
            self.computational_sequences.keys())}
        self.computational_sequences = {}
        for sequence_name in list(new_computational_sequences_data.keys()):
            self.computational_sequences[sequence_name] = computational_sequence(
                sequence_name)
            ...
            if metadata_copy:
                ...
                if sequence_name not in list(old_metadata.keys()):
                    log.error(
                        "Metadata not available to copy ..., please provide metadata before writing to disk later", error=False)
    ...
    def deploy(self, destination, filenames):
        ...
        for seq_key in list(self.computational_sequences.keys()):
            if seq_key not in list(filenames.keys()):
                log.error(
                    "Filename for %s computational sequences not specified" % seq_key)
    ...
    def sort(self, sort_function=sorted):
        ...
        for key in list(self.keys()):
            for csd in list(self[key].keys()):
                ...
                sorted_indices = sort_function(
                    range(this_intervals_np.shape[0]), key=lambda x: this_intervals_np[x, 0])
    ...
    def get_tensors(self, seq_len, non_sequences=[], direction=False, folds=None):
        ...
        if folds == None:
            log.error(
                "No fold specified for get_tensors, defaulting to the first computational sequence ids", error=False)
        ...
        def lpad(this_array, direction):
            if direction == False:
                temp_array = numpy.concatenate([numpy.zeros(
                    [seq_len]+list(this_array.shape[1:])), this_array], axis=0)[-seq_len:, ...]
            else:
                temp_array = numpy.concatenate([this_array, numpy.zeros(
                    [seq_len]+list(this_array.shape[1:]))], axis=0)[:seq_len, ...]
            return temp_array
        ...
        if len(csds) == 0:
            log.error(
                "Dataset is empty, cannot get tensors. Exiting ...!", error=True)
        ...
        for key in list(self[csds[0]].keys()):
            ...
            if which_fold == None:
                log.error("Key %s doesn't belong to any fold ... " %
                          str(key), error=False)
                continue
    ...
    def __intersect_and_copy(self, ref, relevant_entry, epsilon):
        ...
        where_intersect = (numpy.all((sub_copy-ref_copy)
                           > (-epsilon), axis=1) == True)
        intersectors = sub[where_intersect, :]
        intersectors = numpy.concatenate([numpy.maximum(intersectors[:, 0], ref[0])[
                                         :, None], numpy.minimum(intersectors[:, 1], ref[1])[:, None]], axis=1)
        intersectors_features = features[where_intersect, :]
        # checking for boundary cases and also zero length
        where_nonzero_len = numpy.where(
            abs(intersectors[:, 0]-intersectors[:, 1]) > epsilon)
    ...
    def __get_relevant_entries(self, reference):
        ...
        for otherseq_key in set(list(self.computational_sequences.keys()))-set([reference]):
            ...
            for key in list(sub_compseq.data.keys()):
                ...
                relevant_entries[otherseq_key][keystripped]["intervals"].append(
                    relev_intervals)
            ...
            for key in list(relevant_entries[otherseq_key].keys()):
                ...
                sorted_indices = sorted(
                    range(relev_intervals_np.shape[0]), key=lambda x: relev_intervals_np[x, 0])
